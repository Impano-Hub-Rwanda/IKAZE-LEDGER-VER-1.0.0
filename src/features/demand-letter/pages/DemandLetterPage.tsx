...
export function DemandLetterPage() {
  ...
  const loadSavedLetters = useCallback(async () => {
    try {
      ...
      const res = await db.query<SavedDemandLetter>(
        `SELECT id, reference, customer_id, customer_name, letter_date, deadline, subject, body_html, closing, admin_name, show_debt_table, created_at, updated_at
         FROM demand_letters
  ...
  const previewHtml = useMemo(() => {
    ...
    const recipient = selectedCustomer
      ? `<div class="recipient"><strong>Bwana / Madamu muyobozi wa: ${esc(selectedCustomer.full_name)}</strong>${selectedCustomer.address ? `<span>${esc(selectedCustomer.address)}</span>` : ''}</div>`
      : `<div class="recipient"><strong>Bwana / Madamu muyobozi wa:</strong><span></span></div>`;
  ...
  }, [business, selectedCustomer, selectedDebts, outstanding, showDebtTable, letterDate, deadline, subject, bodyHtml, closing, reference, dl, user, adminName]);
  ...
  const downloadPdf = async () => {
    ...
    try {
      ...
      const recipientText = selectedCustomer ? `Bwana / Madamu muyobozi wa: ${selectedCustomer.full_name}` : 'Bwana / Madamu muyobozi wa: ____________________________';
    ...
    try {
      ...
      await db.query(
        `INSERT INTO demand_letters (
          reference, customer_id, customer_name, letter_date, deadline, subject, body_html, closing, admin_name, show_debt_table, user_id, updated_at
        ...
        ON CONFLICT (reference) DO UPDATE SET
          ...
          show_debt_table=EXCLUDED.show_debt_table, user_id=EXCLUDED.user_id, updated_at=now()`,
        [
          reference.trim(),
  ...
  }, [user, saving, reference, selectedCustomer, letterDate, deadline, subject, bodyHtml, closing, adminName, showDebtTable, loadSavedLetters, dl]);
