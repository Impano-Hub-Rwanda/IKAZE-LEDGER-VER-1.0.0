  ...
  const generate = useCallback(async () => {
    ...
    try {
      ...
      if (reportType === 'customer_statement') {
        ...
        const custRes = await db.query<{ full_name: string; phone: string | null; address: string | null }>(
          'SELECT full_name, phone, address FROM customers WHERE id = $1', [customerId],
        );
          ...
          {result.customerInfo && (
            <div className="rounded-xl border border-teal-200 bg-teal-50 p-5 dark:border-teal-800 dark:bg-teal-900/20">
              ...
              <div className="mt-3 grid grid-cols-2 gap-4 md:grid-cols-4">
                ...
                <div>
                  <p className="text-xs font-medium text-teal-700 dark:text-teal-400">{tr.colPhone}</p>
                  <p className="text-sm font-semibold text-teal-900 dark:text-teal-200">{result.customerInfo.phone}</p>
                </div>
                <div>
